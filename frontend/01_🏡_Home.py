...
from views.messages import AUTHORS, CYBORG, INTRO, SIDE_INFO, TITLE, FEATURES
...
def app():
    ...
    st.markdown("Hate Guardian is an AI-powered hate speech detection tool. It allows users to analyze online comments and detect hate speech.")

    st.header("Installation")
    st.markdown("The project consists of a frontend built with Streamlit and a backend built with FastAPI.")
    ...
    st.markdown("Hate Guardian allows users to input a YouTube video URL. It will scrape the comments for that video and run them through a hate speech classification model. The results are displayed in a Streamlit dashboard showing each comment and whether it was classified as hate speech.")

    st.markdown("The goal is to provide an easy way for people to analyze online content and detect hate speech. This allows identifying toxic comments so that action can be taken, such as filtering or removing them.")

    st.markdown("The project uses a DistilBERT model fine-tuned on a dataset of English YouTube comments labeled for hate speech. The model achieves over 95% accuracy on a test set.")
    ...
    st.write("")
    st.markdown("Example:")
    st.code('python controllers/print_terminal.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ"')

    st.write("")
    st.markdown("This will scrape the comments from the YouTube URL, make toxicity predictions on each comment using the ML model, and print out the results.")
    ...
    for contributor in contributors:
        st.markdown(f"<br/><img src='{contributor['avatar']}' width='100px' class='contributors' alt='{contributor['name']}'><sub><b>{contributor['name']}</b></sub>", unsafe_allow_html=True)
